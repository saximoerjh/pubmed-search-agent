...
class PubMedSearch:
    ...
    def __init__(self, email: str = os.getenv("PUBMED_EMAIL"), tool: str = "retrieve_tool", api_key: Optional[str] = os.getenv("PUBMED_API_KEY")):
    ...
    async def search_articles(self, query: str, max_results: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                # 关键：在 esearch 阶段过滤出带 PMC 或 DOI 的记录 AND free full text[Filter]
            ...
            for pmid in pmids:
                ...
                try:
                    ...
                    article = {
                        ...
                        "abstract": self._get_xml_text(article_root,
                                                       './/Abstract/AbstractText') or "No abstract available",
