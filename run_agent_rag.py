...
from inference_utils import chat_without_model
...
def run_generation(sequences_needing_generation, llm):
    # print(sequences_needing_generation[0]["prompt"])
    result = chat_without_model(sequences_needing_generation[0]["prompt"], llm)
    return [result]
...
def rag_agent_main(query, llm):
    ...
    while True:
        ...
        if sequences_needing_generation:
            ...
            outputs = run_generation(sequences_needing_generation, llm)
            print("*"*100)
            print(outputs[0])
            print("*" * 100)
...
if __name__ == '__main__':
    ...
    message = "search recent years' research on the relationship between cancer and obesity, and give me a detailed analysis."
    response = rag_agent_main(message, llm)
    # 提取最终答案 \boxed{...}
    final_answer = extract_between(response, r"\boxed{\text{", "}}")
    if final_answer:
        print("Final Answer:")
        print(final_answer)
    else:
        print("Final Answer not found in the expected format. Full response:")
        print(response)
